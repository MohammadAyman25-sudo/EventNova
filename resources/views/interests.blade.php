...
<x-app-layout>
    <div class="text-center mb-12">
        ...
        <h1 class="text-5xl font-bold text-gray-900 dark:text-white mb-4">{{ __('What Interests You?') }}</h1>
        <p class="text-xl text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">{{ __('Select your interests so we can notify you about events you\'ll love. Choose at least 3 categories.') }}</p>
    </div>
    <form action="{{ route('submit.interest', []) }}" method="POST">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-xl p-8 mb-8">
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">{{ __('Notification Preferences') }}</h2>
            <p class="text-gray-600 dark:text-gray-400 mb-6">{{ __('Choose how you\'d like to recieve updates about events matching your interests.') }}</p>
            <div class="space-y-4">
