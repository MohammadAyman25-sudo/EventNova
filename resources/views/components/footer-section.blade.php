...
<div>
    <h3 class="text-sm sm:text-base font-semibold text-gray-900 dark:text-white mb-3 sm:mb-4">{{ __($title) }}</h3>
    @if(count($icons) == 0)
        <ul class="space-y-2">
            @foreach($links as $link)
                <li><a href="{{ $link['href'] }}" class="text-xs sm:text-sm text-gray-600 dark:text-gray-400 hover:text-purple-600 dark:hover:text-purple-400  transition-colors cursor-pointer">{{ __($link['text']) }}</a></li>
            @endforeach
        </ul>
    @else
